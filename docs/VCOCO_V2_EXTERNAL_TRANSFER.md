...
# Improving Person-Level External Transfer on V-COCO
...
The repository passed Ruff, 85 tests, and the release validator before the selection
lock. The test gate and portable evidence are validated again after export.
...
DINOv3 was not included because the applicable checkpoints required gated access in
the execution environment. The protocol did not bypass that access control or replace
the declared model with an unverified checkpoint.
